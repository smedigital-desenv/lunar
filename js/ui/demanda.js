...
// A troca para os serviços reais é pontual quando a autenticação entrar.
// =====================================================================

import {
  escapeHtml, fmtDataHora, fmtPrazo, badgeSituacao, badgePrioridade,
  badgeSigilo, rotuloSituacao, iniciais, itemTimeline, toast
} from './componentes.js';

// Níveis para decidir ações (espelha perfis.nivel do banco; só UX, o RLS manda).
const NIVEL = { agente_administrativo: 1, chefe_secao: 2, gerente: 3,
                subsecretario: 4, gabinete: 5, admin_ti: 3 };

// ---------------------------------------------------------------- Dados exemplo
const DADOS_EXEMPLO = {
  usuario: { id: 'u1', nome: 'Eduardo Gerente', perfil: 'gerente' },
  demanda: {
  ...
  movimentacoes: [
    { id: 'm1', tipo: 'criacao', texto: 'Demanda criada.', autor_nome: 'Ana Secretária', criado_em: '2026-07-28T12:10:00-03:00' },
    { id: 'm2', tipo: 'encaminhamento', texto: 'Favor apurar com urgência.', autor_nome: 'Ana Secretária', destinatario_nome: 'Gustavo Chefe', criado_em: '2026-07-28T12:12:00-03:00' },
    { id: 'm3', tipo: 'subtarefa', texto: 'Verificar contrato de merenda', autor_nome: 'Gustavo Chefe', criado_em: '2026-07-28T14:30:00-03:00' },
    { id: 'm4', tipo: 'devolutiva', texto: 'Fornecedor confirma entrega para amanhã.', autor_nome: 'Igor Agente', criado_em: '2026-07-29T09:05:00-03:00' },
    { id: 'm5', tipo: 'retificacao', texto: 'Fornecedor confirma entrega para depois de amanhã (correção).', autor_nome: 'Igor Agente', criado_em: '2026-07-29T09:20:00-03:00', movimentacao_retificada_id: 'm4' }
  ]
};
...
// Best-effort com os serviços reais (import dinâmico p/ não quebrar sem config).
async function carregarReal(id) {
  ...
  // Nomes (autor/responsável) virão de views/joins numa próxima etapa.
...
// Árvore recursiva a partir da lista plana (parent_id).
function renderArvore(tarefas) {
  if (!tarefas || !tarefas.length) {
    return '<p class="texto-silencioso">Sem subtarefas.</p>';
  }
...
function renderTimeline(movs) {
  ...
  return ordenadas.map(m => itemTimeline(m, {
    retificadoPor: retificadoPorDe[m.id], ressalva: ressalvaDe[m.id]
  })).join('');
}

// Ações conforme perfil + situação (apenas UX; a segurança é o RLS/funções).
...
function renderAcoes(d, usuario) {
  ...
  return acoes.map(([chave, rotulo]) =>
    `<button class="btn btn-outline-primary" data-acao="${chave}">${escapeHtml(rotulo)}</button>`
  ).join('');
}
...
async function iniciar() {
  ...
  const { demanda, pessoas, tarefas, movimentacoes, usuario, demo } = dados;
  ...
  document.getElementById('timeline').innerHTML = renderTimeline(movimentacoes);
  document.getElementById('acoes').innerHTML = renderAcoes(demanda, usuario || DADOS_EXEMPLO.usuario);

  // Envio real das ações vem na Sessão 7 (modais de tramitação).
  document.getElementById('acoes').addEventListener('click', (e) => {
    const btn = e.target.closest('[data-acao]');
    if (!btn) return;
    toast(`Ação "${btn.textContent}" — envio será implementado na Sessão 7.`, 'aviso');
  });
