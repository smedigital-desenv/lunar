...
- **`042`:** `fn_lic_pode_editar(uuid) → bool` = equipe OU `pode_administrar` — passou a ser a guarda de TODAS as escritas do módulo (`fn_lic_exigir_equipe` e ressalva usam ela); administradores (gabinete, admin_ti) também registram.
